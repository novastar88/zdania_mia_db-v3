...
class Export:
    ...
    def _fetch_records(self):
        to_fetch = int(self.number * 10)
        self.cur.execute(f'''SELECT * FROM export1 LIMIT {to_fetch}''')
        a = self.cur.fetchall()
    ...
    def _add_anki_status(self):
        dbo.AnkiStatusDb(self.pg_con).add_as_exported(
    ...
    def export(self):
        ...
        add_translation_config = self.config["jp_config"]["add_translation_on_export"]
        if add_translation_config == True:
            translation_lang = self.config["jp_config"]["translation_lang"]
            translator = nlp.Translator("JA", translation_lang)
            temp_list = []
